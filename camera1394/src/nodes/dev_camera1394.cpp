...
void Camera1394::readData(sensor_msgs::Image& image, sensor_msgs::Image& right_image)
{
  ...
  switch (videoMode_)
      ...
      if (!DoBayerConversion_)
        {
    	  if (multicam_ == "stereo_interlaced")
    	  {
    		  ...
    		  deinterlace(image, right_image, capture_buffer);
    	  }
    	  else
    	  {
    ...
    default:
      if (dc1394_is_video_mode_scalable(videoMode_))
        {
          format7_.unpackData(image, right_image, capture_buffer);
        }
      else
        {
 ...
 */
void Camera1394::deinterlace(sensor_msgs::Image& left, sensor_msgs::Image& right,
                             unsigned char *buf)
{
  ...
  if (DC1394_SUCCESS != dc1394_deinterlace_stereo(buf, stacked, left.width, left.height * 2))
    {
      free(stacked);
      CAM_EXCEPT(camera1394::Exception, "Unable to deinterlace stereo frame.");
    }
